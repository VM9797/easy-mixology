...
import com.vasmatheus.mokhaiotlhelper.NpcDamaged;
...
@Slf4j
@Singleton
public class DelveGrubHiderService extends DelveService {
  ...
  protected void onHitsplatApplied(HitsplatApplied event) {
    ...
    if (actor instanceof NPC) {
      ...
      DelveGrub grub =
          grubs.stream().filter(n -> n.getNpcIndex() == npcIndex).findFirst().orElse(null);
      if (grub == null) {
        return;
      }
