...
public class StrangeTileOverlay extends Overlay {
    ...
    @Inject
    private SepulchreStrangeTileMarkerConfig config;
    ...
    public void onTick() {
        for (GroundObject groundObject : teleportTickTimeToLiveMap.keySet()) {
            teleportTickTimeToLiveMap.compute(groundObject,
                    (k, currentValue) -> currentValue == null || currentValue <= 0 ? 0 :
    ...
    public Dimension render(Graphics2D graphics2D) {
        ...
        for (GraphicsObject graphicsObject : client.getGraphicsObjects()) {
            ...
            if (strangeTileAtGraphicsObject.isPresent() && (graphicsObject.getId() == 1799 || graphicsObject.getId() == 1815)) {
...
//                        log.info("GRAPHICS OBJECT ANIMATION ID: " + graphicsObject.getAnimation().getId());
//                        log.info("GRAPHICS OBJECT ANIMATION DURATION: " + graphicsObject.getAnimation().getDuration());
//                        log.info("GRAPHICS OBJECT ANIMATION FRAME STEP: " + graphicsObject.getAnimation().getFrameStep());
//                        log.info("GRAPHICS OBJECT ANIMATION NUM FRAMES: " + graphicsObject.getAnimation().getNumFrames());
//                    }
            }
    ...
    private void renderTile(final Graphics2D graphics, Polygon polygon, final Color color, final double borderWidth) {
        if (polygon == null) {
            return;
        }
