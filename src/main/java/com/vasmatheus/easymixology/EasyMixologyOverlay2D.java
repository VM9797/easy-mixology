...
public class EasyMixologyOverlay2D extends OverlayPanel {
  ...
  public Dimension render(Graphics2D graphics) {
    ...
    panelComponent
        .getChildren()
        .add(TitleComponent.builder().text("Easy Mixology").color(Color.GREEN).build());
      ...
      panelComponent
          ...
          .add(
              LineComponent.builder()
                  ...
                  .right(
                      colorCodeString(
                              stats.getPlayerMoxCount() == -1
                                  ? "?"
                                  : formatInt(stats.getPlayerMoxCount()),
                              getMoxColor())
                          + " "
                          + "/ "
                          + colorCodeString(
                              stats.getPlayerAgaCount() == -1
                                  ? "?"
                                  : formatInt(stats.getPlayerAgaCount()),
                              getAgaColor())
                          + " / "
                          + colorCodeString(
                              stats.getPlayerLyeCount() == -1
                                  ? "?"
                                  : formatInt(stats.getPlayerLyeCount()),
                              getLyeColor()))
                  .build());
    }
      ...
      panelComponent
          ...
          .add(
              LineComponent.builder()
                  .left("Target points")
                  .right(
                      colorCodeString(formatInt(stats.getTargetMox()), getMoxColor())
                          + " / "
        ...
        panelComponent
            ...
            .add(
                LineComponent.builder()
                    .left("Target %")
                    .right(
                        colorCodeString(formatInt(stats.getTargetMoxPercent()), getMoxColor())
                            + "% / "
      ...
      panelComponent
          ...
          .add(
              LineComponent.builder()
                  .left("Session points")
                  .right(
                      colorCodeString(formatInt(stats.getSessionMoxCount()), getMoxColor())
                          + " / "
    ...
    if (config.isStationSpeedupOverlayHighlightEnabled()
            && state.getState() == MixologyState.REFINING
  ...
  private void displayOrderInfo() {
    panelComponent
        .getChildren()
        .add(TitleComponent.builder().text("==========================================").build());
    ...
    if (processState == MixologyState.READY_TO_DEPOSIT) {
      panelComponent
          .getChildren()
          .add(TitleComponent.builder().color(Color.GREEN).text("Ready to deposit!").build());
      return;
    }

    panelComponent
        .getChildren()
        .add(LineComponent.builder().left("Potions").right(mapPotionListToString()).build());

    if (processState == MixologyState.MIXING || processState == MixologyState.MIX_READY) {
      panelComponent
          .getChildren()
          .add(
              LineComponent.builder().left("Components").right(mapComponentListToString()).build());
    }

    if (processState == MixologyState.REFINING || processState == MixologyState.READY_TO_REFINE) {
      panelComponent
          .getChildren()
          .add(LineComponent.builder().left("Refinery").right(mapRefineryListToString()).build());
    }
  }
  ...
  private String mapPotionListToString() {
    ...
    return IntStream.range(0, order.potions.size())
        .mapToObj(
            it ->
                it == state.getCurrentlyProcessingPotionIndex()
                    ? colorCodeString(order.potions.get(it).potionName, colorToHex(Color.YELLOW))
                    : order.potions.get(it).shortPotionName)
  ...
  private String mapComponentListToString() {
    ...
    return order.potions.stream()
        .map(
            it ->
                it.componentList.stream()
                    .map(this::colorCodePotionComponentShort)
                    .collect(Collectors.joining("")))
        .collect(Collectors.joining(" / "));
  }
    ...
    return IntStream.range(0, order.refinementTypes.size())
        .mapToObj(
            it ->
                it == state.getCurrentlyProcessingPotionIndex()
                    ? colorCodeString(
                        order.refinementTypes.get(it).actionName, colorToHex(Color.YELLOW))
                    : order.refinementTypes.get(it).actionName)
        .collect(Collectors.joining(" / "));
  }

  private String colorCodePotionComponentShort(PotionComponent component) {
    String color =
        component == PotionComponent.AGA
            ? getAgaColor()
            : component == PotionComponent.LYE ? getLyeColor() : getMoxColor();
