...
public class EasyMixologyOverlay3D extends Overlay {
  ...
  private void outlineRefinery(boolean preDraw, boolean preDrawNext, Graphics2D graphics) {
    ...
    var primaryTargetRefinement =
        preDraw ? state.getFirstTargetRefinementType() : state.getTargetRefinementType();

    var primaryTargetRefinery =
        primaryTargetRefinement == RefinementType.AGITATOR
            ? agitator
            : primaryTargetRefinement == RefinementType.ALEMBIC ? alembic : retort;

    var secondaryTargetRefinement =
        preDrawNext ? state.getNextTargetRefinementType() : RefinementType.NONE;

    var secondaryTargetRefinery =
        secondaryTargetRefinement == RefinementType.AGITATOR
            ? agitator
            : secondaryTargetRefinement == RefinementType.ALEMBIC
                ? alembic
                : secondaryTargetRefinement == RefinementType.RETORT ? retort : null;

    if (secondaryTargetRefinery != null
        && secondaryTargetRefinery != primaryTargetRefinery
        && config.isRefineryPreOutlineEnabled()) {
    ...
    if (config.isRefineryProcessCountEnabled() && !preDraw) {
      ...
      int xOffset =
          primaryTargetRefinement == RefinementType.ALEMBIC
              ? -stationOffset
              : primaryTargetRefinement == RefinementType.RETORT ? (stationOffset / 2) : 0;
      int yOffset =
          primaryTargetRefinement == RefinementType.AGITATOR
              ? (int) ((double) stationOffset * 1.5)
              : stationOffset / 2;
      int zOffset = 250;

      drawTextAtObject(
          primaryTargetRefinery,
          String.format("%dx", state.getRefinementTypeCountMap().get(primaryTargetRefinement)),
          config.refineryOutline(),
          graphics,
          xOffset,
          yOffset,
          zOffset);
    ...
    if (primaryTargetRefinery == alembic
        && config.isRefinerySpeedupOutlineEnabled()
        && uiHelper.isAlembicSpeedupObjectPresent()) {
      outlineObject(primaryTargetRefinery, config.refinerySpeedupOutline());
      return;
    } else if (primaryTargetRefinery == agitator
        && config.isRefinerySpeedupOutlineEnabled()
        && uiHelper.isAgitatorSpeedupObjectPresent()) {
      outlineObject(primaryTargetRefinery, config.refinerySpeedupOutline());
      return;
    }
    ...
    outlineObject(
        primaryTargetRefinery, preDraw ? config.refineryPreOutline() : config.refineryOutline());
  }
  ...
  private void outlineLevers(Graphics2D graphics, boolean preDraw) {
    ...
    for (var component : componentsToAdd) {
      if (component == PotionComponent.LYE
          && lyeLever != null
          && pullCountMap.get(component) != 0) {
        outlineTargetLever(
            lyeLever, preDraw ? config.lyeLeverPreOutline() : config.lyeLeverOutline(), preDraw);

        if (!preDraw) {
          drawLeverPullCount(
              lyeLever, pullCountMap.get(component), config.lyeLeverOutline(), graphics);
        }
      } else if (component == PotionComponent.AGA
          && agaLever != null
          && pullCountMap.get(component) != 0) {
        outlineTargetLever(
            agaLever, preDraw ? config.agaLeverPreOutline() : config.agaLeverOutline(), preDraw);

        if (!preDraw) {
          drawLeverPullCount(
              agaLever, pullCountMap.get(component), config.agaLeverOutline(), graphics);
        }
      } else if (component == PotionComponent.MOX
          && moxLever != null
          && pullCountMap.get(component) != 0) {
        outlineTargetLever(
            moxLever, preDraw ? config.moxLeverPreOutline() : config.moxLeverOutline(), preDraw);

        if (!preDraw) {
          drawLeverPullCount(
              moxLever, pullCountMap.get(component), config.moxLeverOutline(), graphics);
        }
      }
    }
  }
  ...
  private void drawLeverPullCount(
      TileObject targetLever, Integer pullCount, Color color, Graphics2D graphics) {
  ...
  private void drawTextAtObject(
    ...
    var textFont =
        new Font(
            ...
            config.boldText() ? Font.BOLD : originalFont.getStyle(),
  ...
  private void drawConveyorBelt(boolean preDraw) {
    ...
    for (var conveyorBelt : conveyorBelts) {
      outlineObject(
          conveyorBelt, preDraw ? config.conveyorBeltPreOutline() : config.conveyorBeltOutline());
    }
  }
  ...
  private boolean isHopperOutlineNeeded() {
    return HOPPER_THRESHOLD > stats.getHopperMoxCount()
        || HOPPER_THRESHOLD > stats.getHopperAgaCount()
        || HOPPER_THRESHOLD > stats.getHopperLyeCount();
  }
}
