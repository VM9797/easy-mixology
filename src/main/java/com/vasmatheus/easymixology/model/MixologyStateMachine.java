...
public class MixologyStateMachine {
  ...
  @Getter private MixologyOrder order = MixologyOrder.EMPTY;

  @Getter private MixologyState state = MixologyState.WAITING_TO_START;

  @Getter private MixologyVariablesSnapshot variablesSnapshot = MixologyVariablesSnapshot.EMPTY;

  @Getter private boolean isStarted = false;

  @Getter private final Map<PotionComponent, Integer> leversToPullMap = new HashMap<>();

  @Getter private final Map<PotionComponent, Integer> leversToPullNextPotionMap = new HashMap<>();

  @Getter private final Map<RefinementType, Integer> refinementTypeCountMap = new HashMap<>();

  @Getter private int currentlyProcessingPotionIndex = 0;
  ...
  private int getLastPotionIndex() {
    return order.getNumberOfPotionsToDo() - 1;
  }

  public boolean isLastPotion() {
    return currentlyProcessingPotionIndex == getLastPotionIndex();
  }
  ...
  public void updateLeversToPull() {
    ...
    var nextPotion =
        currentlyProcessingPotionIndex == getLastPotionIndex()
            ? null
    ...
    var leversToPull =
        new ArrayList<>(
            Arrays.asList(
                targetPotion.firstComponent,
                targetPotion.secondComponent,
                targetPotion.thirdComponent));

    List<PotionComponent> leversToPullNext =
        nextPotion == null
            ? List.of()
            : new ArrayList<>(
                Arrays.asList(
                    nextPotion.firstComponent,
  ...
  private void processRefiningState(MixologyVariablesSnapshot nextSnapshot) {
    ...
    if (state == MixologyState.READY_TO_DEPOSIT) {
      if (currentlyProcessingPotionIndex < getLastPotionIndex()) {
        state = MixologyState.READY_TO_REFINE;
        currentlyProcessingPotionIndex++;
      } else {
        currentlyProcessingPotionIndex = 0;
